...
class LogAnalyzer:
    ...
    def error_analyze(xml_file, workbook):
        ...
        # init worksheet
        worksheet = LogAnalyzer._init_worksheet_error(workbook)
        # worksheet.set_row(0, 30)
        data_format = LogAnalyzer._data_format(workbook)
        general_error_format = LogAnalyzer._error_msg_format(workbook)
        script_error_format = LogAnalyzer._error_script_msg_format(workbook)
        system_error_format = LogAnalyzer._error_system_msg_format(workbook)
        # list_lang = []
        # list_name = []
        # list_testcase = []
        # list_error = []
    ...
    def _init_worksheet_error(workbook):
        ...
        worksheet.set_column('A:A', LEN_LANGUAGE_NAME)
        # set hearder
        header_format = LogAnalyzer._header_format(workbook)
        LogAnalyzer._write_header(worksheet, header_format)
        return worksheet
    ...
    def error_analyze_for_myaisapp(xml_file, workbook):
        ...
        worksheet = LogAnalyzer._init_worksheet_error_myais(workbook)
        data_format = LogAnalyzer._data_format(workbook)
        general_error_format = LogAnalyzer._error_msg_format(workbook)
        script_error_format = LogAnalyzer._error_script_msg_format(workbook)
        system_error_format = LogAnalyzer._error_system_msg_format(workbook)
            ...
            for lang in menu.findall('./suite'):
                ...
                for ntype in lang.findall('./suite'):
                    ...
                    for testcase in ntype.findall('.//test'):
                        ...
                        error_msg, error_type = LogAnalyzer._analyze_error_msg_for_myaisapp(message)
                        if error_type is "Script":
                        ...
                        if imageAtFail is not None:
                            ...
                            for i in range(len_ListimageName):
                                colum_header = LogAnalyzer.get_column_index_header(start_index_column + i)
    ...
    def _analyze_error_msg_for_myaisapp(msg):
        ...
        ServiceuUavailable = 'Sorry\, the service is unavailable at this time please try again later\.|' + '¢ÍÍÀÑÂ ÃÐººäÁèÊÒÁÒÃ¶ãËéºÃÔ¡ÒÃä´éã¹¢³Ð¹Õé ¡ÃØ³Òà¢éÒãªéºÃÔ¡ÒÃãËÁèÍÕ¡¤ÃÑé§|' + 'Sorry\, the system is temporarily unavailable. Please try again later\.'
    ...
    def _init_worksheet_error_myais(workbook):
        ...
        # set hearder
        header_format = LogAnalyzer._header_format(workbook)
        LogAnalyzer._write_header_myais(worksheet, header_format)
        return worksheet
    ...
    def perf_analyze_for_myaisapp(xml_file, workbook):
        ...
        root = tree.getroot()
        data_format = LogAnalyzer._data_format(workbook)
        fail_format = LogAnalyzer._error_script_msg_format(workbook)
        pass_format = LogAnalyzer._error_system_msg_format(workbook)
    ...
    #   time for create header.
        for i in range(0, round):
            ...
            temp_column_header = LogAnalyzer.get_column_index_header(temp_index)
    ...
    @staticmethod
    def analyze_for_myaisapp_multiexecutors(xml_file, workbook):
        index = INDEX_START
        ...
        worksheet = LogAnalyzer._init_worksheet_success_myais_multiexecutors(workbook)
        data_format = LogAnalyzer._data_format(workbook)
        general_error_format = LogAnalyzer._error_msg_format(workbook)
        script_error_format = LogAnalyzer._error_script_msg_format(workbook)
        system_error_format = LogAnalyzer._error_system_msg_format(workbook)
        ...
        context = etree.iterparse(xml_file, tag='suite')
        libraryList = ["ParallelLocalConfigKeyWord", "BuiltIn", "Collections", "AISAppiumEx", "RedefineKeywords", "Selenium2Library"]
        for action, executor in context:
            ...
            if not (executor.find('status') is None) and (executor.find('status').get('status') == 'PASS'):
                continue

            for menu in executor.findall('./suite'):
                # if status in language not found PASS go to continue.
                # if not (menu.find('status') is None) and (menu.find('status').get('status') == 'PASS'):
                #     continue

                for lang in menu.findall('./suite'):
                    # if status in language not found PASS go to continue.
                    # if not (lang.find('status') is None) and (lang.find('status').get('status') == 'PASS'):
                    #     continue

                    for ntype in lang.findall('./suite'):
                        # if status in n-type not found PASS go to continue.
                        # if not (ntype.find('status') is  None) and (ntype.find('status').get('status') == 'PASS'):
                        #     continue

                        for testcase in ntype.findall('.//test'):
                            column = 7
                            index_column_image = 8 
                            # if status in test case not found PASS go to continue.
                            if (testcase.find('status') is not None) and (
                                        testcase.find('status').get('status') == 'FAIL'):
                                continue
                            ...
                            # Add Keyword Pass in TestCase
                            # KeywordFail = testcase.find('./kw/status[@status="PASS"]/..').get('name')
                            # default msg
                            keyword_pass = "analyze keyword"
                            date_time_pass = "00000000 00:00:00.000"
                            mobile_number = "0000000000"
                            ...
                            for keyword in testcase.findall('./kw'):
                                ...
                                #find keyword
                                if (keyword.get('library') not in libraryList):
                                    keyword_pass = keyword.get('name').lower()
                                    if ("verify" in keyword_pass):
                                        worksheet.write(index - 1, column, keyword_pass, data_format)
                                        column += 2
                                
                                #find image
                                imagePass = keyword.find('.//msg[@html="yes"]')
                                if imagePass is not None:
                                    GetimageName = imagePass.text
                                    ListimageName = re.findall(r'src="screenshot.+.png', GetimageName, re.M | re.I)
                                    imageName = urllib.unquote(ListimageName[0]).decode('utf8')
                                    imageName = imageName.replace('src="','')
                                    # imageName = imageName.replace('%20',' ')
                                    # imageName = imageName.replace('%5B','[')
                                    # imageName = imageName.replace('%2C',',')
                                    # imageName = imageName.replace('%5D',']')

                                    worksheet.set_column(index_column_image, index_column_image, 50)
                                    worksheet.insert_image(index - 1, index_column_image,
                                                           xml_file + '\\..\\' + imageName,
                                                           {'x_scale': 0.4, 'y_scale': 0.4})
                                    index_column_image += 2

                            #find endtime of testcase
                            for time in testcase.findall('./status'):
                                if (time.get('critical') == "yes"):
                                    date_time_pass = time.get('endtime')
                                    worksheet.write('G' + str(index), date_time_pass, data_format)


                            #find teststep
                            for doc in testcase.findall('./doc'):
                                worksheet.write('C' + str(index), doc.text, data_format)

                                                      
                            worksheet.write('E' + str(index), mobile_number, data_format)
                            index += 1
    ...
    def _init_worksheet_success_myais_multiexecutors(workbook):
        worksheet = workbook.add_worksheet(WORKSHEET_NAME)
        worksheet.set_column('O:O', LEN_IMAGE_PASS)
        worksheet.set_column('N:N', LEN_KEYWORD_PASS)
        worksheet.set_column('M:M', LEN_IMAGE_PASS)
        worksheet.set_column('L:L', LEN_KEYWORD_PASS)
        worksheet.set_column('K:K', LEN_IMAGE_PASS)
        worksheet.set_column('J:J', LEN_KEYWORD_PASS)
        ...
        # set hearder
        header_format = LogAnalyzer._header_format(workbook)
        LogAnalyzer._write_header_myais_multiexecutors(worksheet, header_format)
        return worksheet
    ...
    def _write_header_myais_multiexecutors(worksheet, header_format):
        ...
        worksheet.write('J1', "Verify Keyword", header_format)
        worksheet.write('K1', "Image", header_format)
        worksheet.write('L1', "Verify Keyword", header_format)
        worksheet.write('M1', "Image", header_format)
        worksheet.write('N1', "Verify Keyword", header_format)
        worksheet.write('O1', "Image", header_format)
    ...
    def get_column_index_header(index):
        # type: (object) -> object
        return LogAnalyzer._sub_get_column_header(index, index)
    ...
    def _sub_get_column_header(original, index):
        ...
        if (result_div_by_max > maxLen):
            return charArray[result_div_by_max % maxLen - 1] + LogAnalyzer.sub_get_column_header(original, result_div_by_max)
        elif (result_div_by_max >= 1):
            return charArray[result_div_by_max - 1] + LogAnalyzer.sub_get_column_header(original, result_div_by_max % maxLen)
        else:
...
# For Test and Debug.
# workbook1 = xlsxwriter.Workbook("D:\\For Report\\allReport22062017\\workbook22062017.xlsx")
# LogAnalyzer.analyze_for_myaisapp_multiexecutors("D:\\OutputMultiExecutors\\Prod\\outputAllReport\\output.xml", workbook1)
# workbook1.close()
#
